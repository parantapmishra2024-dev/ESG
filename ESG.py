...
def get_answer_with_sources(client, question, report_text=None):
    pdf_context = f"Company Report Context: {report_text[:25000]}\n" if report_text else ""
    prompt = f"""
    Analyze Berlin Packaging for this ESG question. 
    Use the provided report context first. If not found, search Google.
    Question: {question}
    Final Answer:"""
...
if st.button("Start Research") and questions_file and api_key:
    ...
    def format_sources(src):
        if src == "Local Report / General Knowledge": return src
